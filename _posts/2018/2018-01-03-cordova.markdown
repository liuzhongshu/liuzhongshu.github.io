...
这样的话，plugins目录下就会多出通过git下载的插件了，并且插件信息会被加入到config.xml，下次从版本库checkout出来之后，需要用
...
入库的问题在于，当clean项目的时候，重新安装platform，plugin可能和预留的文件冲突，导致编译不过。如果某些插件需要修改之后才能工作，应该在github上单独为插件fork或新建一个项目，用这个新项目地址配置到cordova工程里，下面详述。
