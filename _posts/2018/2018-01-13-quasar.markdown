...
这个模板的repo需要是github上的，如果是非github的，要加-c参数和前缀，并且需要要用vue-init，而不能用vue init（有个bug导致windows下后面两个参数对调）。比如：

```
vue-init -c bitbucket:user/repo projectname 
```

如果不是团队使用，也可以用本地目录代替远端仓库，-c后面跟相对地址就可以了，比如-c ..\template\dir
